---
name: execplan-portability-check
description: >-
  Score an ExecPlan's portability to a fresh implementer with no conversation
  history. Identify implicit references, judgment-requiring steps, missing
  evidence, and non-mechanical validation. Use after execplan-improve and
  before handing the plan off to a different model or session, or any time
  the user asks whether a plan is portable, self-contained, or safe to hand
  off.
---
