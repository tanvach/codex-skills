...
backup_root="$gemini_home/skill-backups/$(date +%Y%m%d-%H%M%S)"
check=false
prune=false
...
if [ "$check" = true ]; then
  printf 'Check mode only: no Gemini CLI skill links were changed.\n'
else
  printf 'Restart Gemini CLI to pick up newly added skills.\n'
fi
