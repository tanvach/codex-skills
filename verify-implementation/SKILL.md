...
description: >-
  Verify an implemented ExecPlan by triangulating the plan against the actual
  git diff and validation runs. Use after implement-execplan completes —
  ideally in a fresh session, or on a stronger model — to catch silent
  deviations, missing edits, scope creep, and validation failures the
  implementer may have glossed over. Especially valuable after a cross-model
  handoff where the implementer was a cheaper or weaker model.
---
